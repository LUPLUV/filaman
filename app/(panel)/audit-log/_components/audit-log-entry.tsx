        ...
        <Card className="w-full">
            <div className="flex flex-row justify-between p-3 cursor-pointer" onClick={() => setOpen(!open)}>
                <div className="flex gap-2 items-center">
                    {log.action?.startsWith("new") ? <Badge variant="default">NEU</Badge> : log.action?.startsWith("update") ? <Badge variant="secondary">UPDATE</Badge> : log.action?.startsWith("delete") ? <Badge variant="destructive">LÖSCHEN</Badge> : log.action?.startsWith("weight") ? <Badge variant="secondary">GEWICHT</Badge> : log.action?.startsWith("status") ? <Badge variant="secondary">GEWICHT</Badge> : <Badge variant="outline">UNBEKANNT</Badge>}
